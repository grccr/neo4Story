...
/***/ function(module, exports, __webpack_require__) {
	...
	var actions = __webpack_require__(41).actions;
	...
	var neo4jFullTextSearch = actions.neo4jFullTextSearch,
	    neo4jFindAllMatches = actions.neo4jFindAllMatches;
	...
	describe('actions', function () {
	    it('neo4jFullTextSearch', function () {
	        var store = {};
	        var payload = { searchRequest: 'Владимир Мединский' };
	        neo4jFullTextSearch(store, payload).then(function (result) {
	            consle.log(result);
	        });
	    });
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	/**
	 * Created by nickvolkov on 04.02.17.
	 */
	...
	module.exports = {
	    state: {
	        ...
	        neo4jLogin: 'neo4j',
	        neo4jPassword: '112358'
	    },
	    mutations: {},
	    actions: {
	        neo4jFullTextSearch: function neo4jFullTextSearch(store, payload) {
	            /**
	            ...
	            var driver = neo4j.driver(store.state.neo4jUrl, neo4j.auth.basic(store.state.neo4jLogin, store.state.neo4jPassword));
	            ...
	            var extraQueryStr = surname ? 'OR a.name STARTS WITH "' + surname + '" OR a.surname STARTS WITH "' + surname + '"' : '';
	            return session.run("MATCH (a) WHERE a.name STARTS WITH {name} OR a.surname STARTS WITH {name} " + "OR a.name STARTS WITH {searchRequest}" + extraQueryStr + "RETURN a", { name: name, surname: surname, searchRequest: searchRequest }).catch(function (error) {
	                session.close();
	                throw error;
	            });
	        },
	        neo4jFindAllMatches: function neo4jFindAllMatches(store, payload) {
	            /**
	             * Use it for find any mantched nodes in neo4j with search request from payload
	             * @param payload: {
	             *     searchRequest: String,
	             *     withEdges: boolean (default: true)
	             * }
	             */
	            var neo4j = window.neo4j.v1;
	            var driver = neo4j.driver(store.state.neo4jUrl, neo4j.auth.basic(store.state.neo4jLogin, store.state.neo4jPassword));
	            var session = driver.session();
	            var withEdges = payload.withEdges || true;

	            store.dispatch('neo4jFullTextSearch', payload).then(function (result) {
	                var matches = result.records.map(function (record) {
	                    return { id: record._fields[0].identity };
	                });
	                return session.run('MATCH p=(a)-[r]-(b) WHERE ID(a) IN {matches} ' + 'UNWIND nodes(p) as allnodes WITH COLLECT(ID(allnodes)) AS ALLID ' + 'MATCH (n)-[r2]-(m) where ID(n) IN ALLID AND ID(m) IN ALLID ' + 'return n, m, r2', { matches: matches });
	            }).catch(function (error) {
	                session.close();
	                throw error;
	            });
	        },
	        neo4jSearchById: function neo4jSearchById(store, payload) {
	            var neo4j = window.neo4j.v1;
	            var driver = neo4j.driver(store.state.neo4jUrl, neo4j.auth.basic(store.state.neo4jLogin, store.state.neo4jPassword));
	            var session = driver.session();
	            var id = payload.id;
	            var update = payload.update;
	            return session.run("MATCH (a:Person)-[b]-(c) WHERE ID(a) = {id}" +
	            // "OPTIONAL MATCH" +
	            "RETURN a,b,c", { id: id }).then(function (result) {
	                if (result.records.length) {
	                    return result;
	                } else {
	                    return session.run("MATCH (a:Person) WHERE ID(a) = {id} RETURN a", { id: id });
	                }
	            }).catch(function (error) {
	                session.close();
	                throw error;
	        ...
	        neo4jResponseParse: function neo4jResponseParse(store, payload) {
	            var graph = {
	                nodes: [],
	                edges: [],
	                nodesMap: {},
	            ...
	            var neo4jData = payload.neo4jData;
	            var update = payload.update;

	            neo4jData.records.forEach(function (res) {
	                res._fields.forEach(function (field) {
	                    if (field) {
	                        if (!field.start) {
	                            var nodeData = (0, _assign2.default)({}, defNode, field.properties);
	                            nodeData.id = field.identity.low;
	                            if (!(nodeData.id in graph.nodesMap)) {
	                                graph.nodesMap[nodeData.id] = nodeData;
	                                nodeData.x = Math.random() * 0.5;
	                                nodeData.y = Math.random() * 0.5;
	                                nodeData.color = colorBrew[Math.round(Math.random() * 5)];
	                                nodeData.size = 10;
	                                nodeData.semantic_type = nodeData.type;
	                                nodeData.image = 'img/pig.png';
	                                nodeData.type = 'image';
	        ...
	        startGraphLayout: function startGraphLayout(store, payload) {
	            store.commit("GRAPH_START_LAYOUT", payload.iters);
	        },
	        stopGraphLayout: function stopGraphLayout(store, payload) {
	            store.commit("GRAPH_STOP_LAYOUT");
	        }
	    },
	    getters: {}
	};
